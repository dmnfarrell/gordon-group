...
ggplottree <- function(tree, meta, cols=NULL, colors=NULL, cmaps=NULL, legends=NULL,
    ...
    p <- p + new_scale_fill() + 
            ...
            scale_fill_manual(values=clrs, na.value="black", guide = guide_legend(override.aes = list(size=legend.pointsize)))
    p2 <- p
    if (length(cols)>1){
    ...
    p2 <- p2 + theme_tree2(legend.text = element_text(size=legend.fontsize), legend.key.size = unit(.7, 'cm'), 
